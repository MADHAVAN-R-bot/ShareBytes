...
export default function NGOClaimHistoryPage() {
  const { user } = useAuth();
    ...
    <DashboardLayout>
      <div className="space-y-6">
        <div>
          ...
          <h1 className="text-2xl font-bold text-on-surface">Claim History</h1>
          <p className="text-xs text-on-surface-variant font-medium mt-1">All batch allocations and claims made by your organisation.</p>
        </div>

        {claims.length === 0 ? (
          <div className="bg-white rounded-3xl p-12 text-center border border-[#e1bfb5]/40 space-y-3">
            <span className="material-symbols-outlined text-gray-300 text-[48px]">history</span>
            <p className="text-xs font-bold text-on-surface-variant">No claims history yet. Browse the surplus feed to get started.</p>
          </div>
        ) : (
            ...
            {claims.map(c => (
              <div key={c.id} className="bg-white p-5 rounded-2xl border border-[#e1bfb5]/40 shadow-sm flex items-center justify-between gap-4">
                <div className="space-y-1">
                  <p className="text-sm font-bold text-on-surface">{c.listing?.title || 'Batch Allocation'}</p>
                  <p className="text-xs text-on-surface-variant">{c.portion_count} portion(s) • {c.listing?.is_donation_only ? 'Free Donation' : `₹${(c.listing?.discounted_price || 0) * c.portion_count}`}</p>
