            ...
            {pending.map(p => (
              <div key={p.id} className="bg-white p-5 rounded-2xl border border-[#e1bfb5]/40 shadow-sm space-y-3">
                ...
                <div className="flex gap-2">
                  ...
                  <button onClick={() => handleVerify(p.id, 'rejected', 'Documents do not meet FSSAI requirements')} className="px-4 py-2 rounded-full bg-[#ffdad6] text-[#93000a] text-xs font-bold hover:opacity-80">✗ Reject</button>
                </div>
              </div>
            ))}
          </div>
