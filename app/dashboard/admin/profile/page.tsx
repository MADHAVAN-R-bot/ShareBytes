...
export default function AdminProfilePage() {
  ...
  const handleSave = (e: React.FormEvent) => {
    ...
    setIsSaving(true);
    setTimeout(() => { updateProfile({ full_name: fullName, phone, address }); setIsSaving(false); showToast('Admin profile updated!', 'success'); }, 600);
  };
    ...
    <DashboardLayout>
      <div className="space-y-8 max-w-2xl">
        <div>
          ...
          <h1 className="text-2xl sm:text-3xl font-bold text-on-surface">Admin Profile</h1>
          <p className="text-xs text-on-surface-variant font-medium mt-1">Manage your administrator account information.</p>
        </div>
        ...
        <div className="bg-white rounded-3xl p-6 border border-[#e1bfb5]/40 shadow-sm flex items-center gap-5">
          ...
          <div>
            ...
            <span className="mt-1 inline-block px-2.5 py-0.5 rounded-full bg-primary/10 text-primary text-[11px] font-bold">Platform Administrator</span>
          </div>
        </div>

        <form onSubmit={handleSave} className="bg-white rounded-3xl p-6 border border-[#e1bfb5]/40 shadow-sm space-y-5">
          <h2 className="text-base font-bold text-on-surface">Account Information</h2>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <div>
              ...
              <input type="tel" value={phone} onChange={e => setPhone(e.target.value)} placeholder="+91 98765 00000" className="w-full px-4 py-3 rounded-2xl bg-surface-container-low border border-[#e1bfb5]/40 text-xs font-medium focus:outline-none focus:ring-2 focus:ring-primary" />
            </div>
            <div>
              <label className="block text-xs font-bold text-on-surface mb-1">Office Address</label>
              <input type="text" value={address} onChange={e => setAddress(e.target.value)} placeholder="Central Tech Hub, Chennai" className="w-full px-4 py-3 rounded-2xl bg-surface-container-low border border-[#e1bfb5]/40 text-xs font-medium focus:outline-none focus:ring-2 focus:ring-primary" />
            </div>
          </div>
          <div className="p-4 rounded-2xl bg-surface-container-low border border-[#e1bfb5]/40">
            <p className="text-xs font-bold text-on-surface">Role</p>
            <p className="text-xs text-on-surface-variant mt-0.5">Platform Administrator — Full access to all platform controls, verifications, and user management.</p>
          </div>
