...
export default function RestaurantOrdersPage() {
  const { user } = useAuth();
  const claims: FoodClaim[] = user ? DataService.getClaimsForListingOwner(user.id) : [];
    ...
    <DashboardLayout>
      <div className="space-y-6">
        <div>
          ...
          <h1 className="text-2xl font-bold text-on-surface">Incoming Orders</h1>
          <p className="text-xs text-on-surface-variant font-medium mt-1">All claims received on your listings — accept or decline each one.</p>
        </div>
        {claims.length === 0 ? (
          <div className="bg-white rounded-3xl p-12 text-center border border-[#e1bfb5]/40 space-y-3">
            ...
            <p className="text-xs font-bold text-on-surface-variant">No orders yet. Create a listing to start receiving claims.</p>
          </div>
        ) : (
            ...
            {claims.map(c => (
              <div key={c.id} className="bg-white p-5 rounded-2xl border border-[#e1bfb5]/40 shadow-sm flex items-center justify-between gap-4">
                <div className="space-y-1">
                  ...
                  <p className="text-xs text-on-surface-variant">By: {c.claimer?.full_name || c.claimed_by} • {c.portion_count} portion(s)</p>
                ...
                <div className="flex items-center gap-2">
                  ...
                  {c.status === 'pending' && (
                    <div className="flex gap-1">
                      <button onClick={() => { DataService.updateClaimStatus(c.id, 'accepted'); window.location.reload(); }} className="px-3 py-1.5 rounded-full bg-[#eaf4ee] text-[#005236] text-[11px] font-bold hover:opacity-80">Accept</button>
                      <button onClick={() => { DataService.updateClaimStatus(c.id, 'declined'); window.location.reload(); }} className="px-3 py-1.5 rounded-full bg-[#ffdad6] text-[#93000a] text-[11px] font-bold hover:opacity-80">Decline</button>
                    </div>
                  )}
                </div>
              </div>
