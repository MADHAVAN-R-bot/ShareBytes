...
-- 1. Helper Function for Admin Check (Prevents RLS Recursion)
...
CREATE POLICY "Profiles viewable by owner or admin"
  ...
  USING (auth.uid() = id OR public.is_admin() OR role IN ('restaurant', 'ngo', 'food_donor'));
...
CREATE POLICY "Claims viewable by claimer, listing owner, or admin"
  ...
  USING (
    claimed_by = auth.uid()
    OR EXISTS (SELECT 1 FROM public.listings WHERE id = claims.listing_id AND created_by = auth.uid())
    OR public.is_admin()
  );
