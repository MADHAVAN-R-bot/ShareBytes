-- =============================================================================
-- ShareBytes — Complete & Consolidated Supabase Database Setup
-- File: supabase/FINAL_SETUP.sql
-- 
-- Run this script top to bottom in the Supabase SQL Editor for a brand-new
-- or existing database. It is fully idempotent and builds all tables,
-- triggers, helper functions, RLS security policies, and storage settings.
-- =============================================================================

-- -----------------------------------------------------------------------------
-- STEP 1: Core Tables & Constraint Updates
-- -----------------------------------------------------------------------------

-- 1a. Profiles Table (must come FIRST — all other custom tables FK-reference it)
-- Note: FK points to auth.users(id) so every valid profile must correspond to an Auth identity.
CREATE TABLE IF NOT EXISTS public.profiles (
  id                    UUID PRIMARY KEY REFERENCES auth.users(id) ON DELETE CASCADE,
  email                 TEXT NOT NULL,
  role                  TEXT NOT NULL CHECK (role IN ('customer', 'restaurant', 'food_donor', 'donor', 'ngo', 'admin')),
...
-- Ensure profiles_role_check constraint allows canonical roles ('customer', 'restaurant', 'food_donor', 'ngo', 'admin')
-- as well as the 'donor' alias to prevent 23514 check constraint errors during manual inserts or legacy payloads.
ALTER TABLE public.profiles
  DROP CONSTRAINT IF EXISTS profiles_role_check;

ALTER TABLE public.profiles
  ADD CONSTRAINT profiles_role_check
  CHECK (role IN ('customer', 'restaurant', 'food_donor', 'donor', 'ngo', 'admin'));
...
-- -----------------------------------------------------------------------------
-- STEP 2: Functions & Automatic Profile Creation Trigger
-- -----------------------------------------------------------------------------
...
-- 2b. handle_new_user() — Trigger function to auto-create matching public.profiles row on auth.users signup.
-- This function runs with SECURITY DEFINER privileges to ensure profile creation succeeds regardless of RLS state.
...
DECLARE
  _role TEXT;
BEGIN
  -- Extract role from metadata sent by AuthContext signup() options.data
  _role := COALESCE(NEW.raw_user_meta_data->>'role', 'customer');

  -- Normalize 'donor' alias to canonical 'food_donor'
  IF _role = 'donor' THEN
    _role := 'food_donor';
  END IF;

  -- Fallback to 'customer' if role is not in allowed list
  IF _role NOT IN ('customer', 'restaurant', 'food_donor', 'donor', 'ngo', 'admin') THEN
    _role := 'customer';
  END IF;
  ...
  ON CONFLICT (id) DO UPDATE SET
    ...
    fssai_cert_url = EXCLUDED.fssai_cert_url,
    registration_cert_url = EXCLUDED.registration_cert_url,
    entity_photo_url = EXCLUDED.entity_photo_url,
    updated_at = NOW();
...
-- Grant execution privileges on handle_new_user()
GRANT EXECUTE ON FUNCTION public.handle_new_user() TO postgres, anon, authenticated, service_role;

-- Re-create the trigger on auth.users to guarantee automatic profile creation
...
-- -----------------------------------------------------------------------------
-- STEP 4: Row Level Security (RLS) Policies (Clean Reverted State)
-- -----------------------------------------------------------------------------
...
CREATE POLICY "Profiles viewable by owner or admin"
  ...
  USING (
    auth.uid() = id
    OR public.is_admin()
    OR role IN ('restaurant', 'ngo', 'food_donor', 'donor')
  );
